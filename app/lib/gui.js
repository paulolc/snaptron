...
IDE_Morph.prototype.openIn = function (world) {
    ...
    */
    function interpretUrlAnchors() {
        var dict;
        if (location.hash.substr(0, 6) === '#load:') {
...
IDE_Morph.prototype.createControlBar = function () {
  ...
  //      serialSelectButton,
    ...
    button.contrast = this.buttonContrast;
    button.drawNew();
    // button.hint = 'serial port operations';
    button.fixLayout();
    connectMbotButton = button;
    this.controlBar.add(connectMbotButton);
    this.controlBar.connectMbotButton = connectMbotButton; // for menu positioning
/*
    //serialSelectButton
    button = new PushButtonMorph(
        this,
    ...
    // button.hint = 'serial port operations';
    button.fixLayout();
    serialSelectButton = button;
    this.controlBar.add(serialSelectButton);
    this.controlBar.serialSelectButton = serialSelectButton; // for menu positioning
*/
    // projectButton
    button = new PushButtonMorph(
        this,
...
//        [stopButton, pauseButton, startButton, connectMbotButton, serialSelectButton ].forEach(
        [stopButton, pauseButton, startButton, connectMbotButton ].forEach(
            function (button) {
...
/*
        connectMbotButton.setCenter(myself.controlBar.center());
        connectMbotButton.setLeft(this.left() );

        serialSelectButton.setCenter(myself.controlBar.center());
        serialSelectButton.setRight(connectMbotButton.left() - padding);
*/
        settingsButton.setCenter(myself.controlBar.center());
        settingsButton.setRight(this.left() - padding);
...
//        this.label.setLeft(this.connectMbotButton.right() + padding);
...
IDE_Morph.prototype.fixLayout = function (situation) {
    ...
    if (situation !== 'refreshPalette') {
        ...
        if( this.spriteBar.left() > 0 ) {
            this.categories.setWidth(this.spriteBar.left()-padding);            
        }
    }
    ...
    if( this.spriteBar.left() > 0 ) {
        this.palette.setWidth(this.spriteBar.left()-padding);
    }
    ...
    if (situation !== 'refreshPalette') {
        ...
        if (this.isAppMode) {
            ...
            this.spriteBarHandle.fixLayout();
        }


        this.spriteBar.setPosition(this.categories.topRight().add(padding));
...
IDE_Morph.prototype.projectMenu = function () {
    ...
    function createMediaMenu(mediaType, loadFunction) {
        return function () {
            var ide = this;
            this.getMediaList(mediaType, function( names ){
    ...
    menu.addItem(
        ...
        createMediaMenu(
            ...
            function loadCostume(file, name) {                              
                ...
                myself.loadImg( url, function( canvas ){
...
IDE_Morph.prototype.getMediaList = function (dirname, callback) {
    ...
    this.getURL(url, function( err, responseText ){ 
        if( err ){
            //TODO: Display error message
            return;
        }
        data = ide.parseResourceFile( responseText );
...
IDE_Morph.prototype.openProject = function (name) {
    log( 'IDE_Morph.prototype.openProject('+name+')');
    var str;
    if (name) {
...
IDE_Morph.prototype.toggleAppMode = function (appMode) {
    var world = this.world(),
        elements = [
            ...
            //this.controlBar.serialSelectButton,
            //this.controlBar.connectMbotButton,
...
// IDE_Morph asynchronous HTTP data fetching

IDE_Morph.prototype.getURL = function (url, cb) {
...
ProjectDialogMorph.prototype.setSource = function (source) {
    ...
    case 'examples':
        msg = myself.ide.showMessage('Updating\nproject list...');
        this.getExamplesProjectList( function( projectList ){
            myself.projectList = projectList; 
            myself.arrangeProjectList();
        });
        break;
    case 'local':
...
ProjectDialogMorph.prototype.arrangeProjectList = function () {
    var myself = this;
...
ProjectDialogMorph.prototype.openProject = function () {
    var self = this;
    var proj = self.listField.selected,
        src;
    if (!proj) {return; }
    self.ide.source = self.source;
    if (self.source === 'cloud') {
        self.openCloudProject(proj);
    } else if (self.source === 'examples') {
        ...
        self.ide.openProject(proj.name);
        self.destroy();
    }
};
...
StageHandleMorph.prototype.mouseDownLeft = function (pos) {
    ...
    this.step = function () {
        ...
        if (world.hand.mouseButton) {
            ...
            if( myself.target.dimensions ){ 
                ide.stageRatio = newWidth / myself.target.dimensions.x;
            } else {
                myself.target.setLeft( newPos );
                myself.target.setWidth( newWidth );
            }
