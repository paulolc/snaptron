...
SpriteMorph.prototype.blockColor[' mBot'] = new Color(24, 136, 24);
...
SpriteMorph.prototype.initMbotBlocks = function () {

    this.blocks.mBotReportAnalogReading = 
    {
        only: SpriteMorph,
        type: 'reporter',
        category: ' mBot',
        spec: 'mBot - analog reading %analogPin',
        translatable: true
    };

    this.blocks.mBotReportDigitalReading = 
    {
        only: SpriteMorph,
        type: 'predicate',
        category: ' mBot',
        spec: 'mBot - digital reading %digitalPin',
        translatable: true
    };

    this.blocks.mBotConnectArduino =
    {
        only: SpriteMorph,
        type: 'command',
        category: ' mBot',
        spec: 'mBot - connect arduino at %s'
    };

    // Keeping this block spec, although it's not used anymore!
    this.blocks.mBotSetPinMode =
    {
        only: SpriteMorph,
        type: 'command',
        category: ' mBot',
        spec: 'mBot - setup digital pin %digitalPin as %pinMode',
        defaults: [null, 'servo'],
        translatable: true
    };

    this.blocks.mBotDigitalWrite =
    {
        only: SpriteMorph,
        type: 'command',
        category: ' mBot',
        spec: 'mBot - set digital pin %digitalPin to %b',
        translatable: true
    };

    this.blocks.mBotServoWrite =
    {
        only: SpriteMorph,
        type: 'command',
        category: ' mBot',
        spec: 'mBot - set servo %servoPin to %servoValue',
        defaults: [null, 'clockwise'],
        translatable: true
    };

    this.blocks.mBotPwmWrite =
    {
        only: SpriteMorph,
        type: 'command',
        category: ' mBot',
        spec: 'mBot - set PWM pin %pwmPin to %n',
        translatable: true
    };
...
*/
        blocks.push('-');
        blocks.push(blockBySelector('mBotServoWrite'));
        blocks.push(blockBySelector('mBotDigitalWrite'));
        blocks.push(blockBySelector('mBotPwmWrite'));
        blocks.push('-');
        blocks.push(blockBySelector('mBotReportAnalogReading'));
        blocks.push(blockBySelector('mBotReportDigitalReading'));
    };
